"""
    ...
    В первой строке подаётся число 𝑛 – количество холодильников. В последующих 𝑛 строках вводятся 
...
for _ in range(n):
    ...
    if 'a' in s:
        if 'n' in s:
            if 't' in s:
                if 'o' in s:
                    if s.find('a') < s.find('n', s.find('a')) < s.find('t', s.find('n')) < s.find('o', s.find('t')) < s.rfind('n', s.find('t')):
                        print(count, end=' ')